...
def main():
    ...
    if args.IDlogo:
        info_height_dic = {}
        for key in results:  # this result contains functionLogoResult for both: homo and trytyp
        ...
        # in id_info1 key_b is the Background specie
        # in id_info2 key_f is the Background specie
        ...
        # put both of them in a for loop
        ...
        # callIdLogoHeight(self, height_dict1, height_dict2, info):
        ...
        resultsKLD = {}
        for key in logo_dict:
            prior, post = logo_dict[key].calculate_prob_dist(types)
            resultsKLD[key] = {}
            resultsKLD[key]['post'] = post
            resultsKLD[key]['prior'] = prior

        prior, post = logo_dict[key_1].calculate_prob_dist(types)
        resultsKLD[key_1] = {}
        resultsKLD[key_1]['post'] = post
        resultsKLD[key_1]['prior'] = prior

        prior, post = logo_dict[key_2].calculate_prob_dist(types)
        resultsKLD[key_2] = {}
        resultsKLD[key_2]['post'] = post
        resultsKLD[key_2]['prior'] = prior

        # # homo is background TryTryp is foreground key1 is replaced for Homo
        kld_info, kld_height = logo_dict[key_1].calculate_kld(resultsKLD[key_1]['prior'],
                                                            resultsKLD[key_2]['prior'],
                                                            resultsKLD[key_1]['post'], resultsKLD[key_2]['post'])
